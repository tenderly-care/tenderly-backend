const axios = require('axios');

async function testSessionUnique() {
  const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..SBCR4NrS7WUNG62XEDZpN7p1JjHxt3f-hEujTA96Pfc';
  
  const headers = {
    'Authorization': `Bearer ${token}`,
    'Content-Type': 'application/json'
  };

  try {
    console.log('🔍 Testing consultation creation with unique session_id...');
    
    // Generate a unique session_id using timestamp
    const uniqueSessionId = `session_test_${Date.now()}`;
    
    const consultationData = {
      patientId: '6884d6abc1ceb202ca8066c4',
      session_id: uniqueSessionId,
      consultationType: 'chat',
      paymentInfo: {
        paymentId: 'mock_pay_test_123',
        amount: 299,
        currency: 'INR',
        paymentMethod: 'online',
        paymentStatus: 'completed',
        transactionId: 'mock_pay_test_123',
        paymentDate: new Date()
      },
      metadata: {
        ipAddress: '127.0.0.1',
        userAgent: 'test-agent',
        location: 'test',
        deviceInfo: 'test'
      }
    };

    console.log('Consultation data with unique session_id:', JSON.stringify(consultationData, null, 2));
    
    const response = await axios.post('http://localhost:3000/api/v1/consultations', consultationData, { headers });
    console.log('✅ Consultation creation successful:', response.data);

  } catch (error) {
    console.error('❌ Error:', error.response?.data || error.message);
    if (error.response?.status) {
      console.error('Status:', error.response.status);
    }
  }
}

testSessionUnique(); 