...
export class User {
  ...
  @Prop({ type: String, enum: AccountStatus, default: AccountStatus.PENDING_VERIFICATION })
...
UserSchema.index({ 'professionalInfo.medicalLicenseNumber': 1 }, { sparse: true });
...
UserSchema.methods.canLogin = function(): { canLogin: boolean; reason?: string } {
  if (this.isAccountLocked()) {
    return { canLogin: false, reason: 'Account is temporarily locked due to failed login attempts' };
  }
  ...
  if (this.requiresMFA()) {
    if (this.accountStatus === AccountStatus.PENDING_MFA_SETUP) {
      return { canLogin: false, reason: 'Please complete MFA setup before logging in' };
    }
...
UserSchema.methods.incrementFailedLogin = function(): Promise<void> {
  ...
  if (this.failedLoginAttempts >= maxAttempts) {
    this.accountLockedUntil = new Date(Date.now() + lockoutDuration * 60 * 1000);
  }
...
UserSchema.methods.isTrustedDevice = function(deviceFingerprint: string): boolean {
...
UserSchema.set('toJSON', {
  transform: function(doc, ret) {
    ...
    const sensitiveFields = ['password', 'mfaSecret', 'mfaBackupCodes', 'passwordHistory', 'passwordResetToken', '__v'];
    ...
    if (ret.kycDocuments) {
      const sensitiveKycFields = ['aadhaarNumber', 'panNumber', 'drivingLicense'];
